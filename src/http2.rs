...
pub fn parse_bit_and_u31(i: &[u8]) -> IResult<&[u8], (u8, u32)> {
    // first, parse a u32:
    let (i, x) = be_u32(i)?;
...
fn test_pack_and_parse_bit_and_u31() {
    ...
    for &(bit, number) in &test_cases {
        ...
        let (_, (parsed_bit, parsed_number)) = parse_bit_and_u31(&packed).unwrap();
...
impl PrioritySpec {
    ...
    fn write(self, mut slice: &mut [u8]) -> std::io::Result<()> {
...
pub struct GoAway<'a> {
    ...
    pub additional_debug_data: &'a [u8],
}

impl<'a> GoAway<'a> {
    pub fn parse(i: &'a [u8]) -> IResult<&[u8], Self> {
        ...
        Ok((
            ...
            Self {
                ...
                additional_debug_data: rest,
            },
        ))
    }

    fn into_piece(self, mut slice: &mut [u8]) -> std::io::Result<()> {
